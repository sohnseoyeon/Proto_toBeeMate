// 전체 코드: 잔상 남는 ripple + 누적 왜곡 효과 + 기존 동작 통합
...
const rippleEffectMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    vec2 ripple(vec2 uv, vec2 center, float age) {
      ...
      float fade = clamp(1.0 - age / 12.0, 0.0, 1.0);  // 감쇠 속도 매우 느리게
      float mask = pow(1.0 - smoothstep(radius * 0.5, radius, dist), 2.0); // 더 넓게, 더 부드럽게
      float wave = 0.1 * sin(30.0 * dist - age * 3.0) / (1.0 + 80.0 * dist); // 진폭도 키움
...
function loadCompositeTexture(index) {
  ...
  bgImg.onload = shadowImg.onload = () => {
    if (++loaded === 2) {
      ...
      const shadowW = poster.width * 0.24;
      const shadowH = shadowW / aspect;
      ctx.drawImage(
        shadowImg,
        poster.x + poster.width * 0.2,
        poster.y + poster.height * 0.65,
        shadowW,
        shadowH
      );
...
window.addEventListener("pointermove", (e) => {
  ...
  for (let i = 0; i < maxRipples; i++) {
    uniforms.uRipples.value[i] =
      rippleHistory[i] || new THREE.Vector3(-1, -1, 0);
  }
